 ...
 */
public abstract class ZZZNetworkRequest<T> implements Callback{
    T result;
    int code;
    String errorMessage;
    Throwable exception;
    ZZZNetworkManager.OnResultListener<T> listener;

    public void setOnResultListener(ZZZNetworkManager.OnResultListener<T> listener) {
    ...
    private void sendSuccess(T result) {
        this.result = result;
        ZZZNetworkManager.getInstance().sendSuccess(this);
    }
    ...
    protected void sendError(int code, String errorMessage, Throwable exception) {
        ...
        ZZZNetworkManager.getInstance().sendFail(this);
    }
